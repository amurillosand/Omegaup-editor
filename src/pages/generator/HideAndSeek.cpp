...
int main() {
  ...
  int n = random.get<int>(5, N);
  int m = random.get<int>(5, N);

  cout << n << " " << m << '\n';

  int k = random.get<int>(2, min(n * m, K)) / 2;
  set<ii> where;
  while (k--) {
    int r = random.get<int>(1, n);
    int c = random.get<int>(1, m);
    where.emplace(r, c);
  }

  int row = random.get<int>(1, n);
  int col = random.get<int>(1, m);
  where.erase({row, col});
